...
#SBATCH --nodes=2                          # 需要1个节点
#SBATCH --ntasks-per-node=1                # 保持原有的4个任务配置
#SBATCH --cpus-per-task=30                 # 每个任务使用30个CPU核心
#SBATCH --mem=200G                         # 每个节点分配800GB内存
#SBATCH --gres=gpu:h100-47:1               # 保持申请4块MIG实例不变
#SBATCH --time=5:00:00                  # 最长运行时间
...
accelerate launch --num_machines 2 --num_processes 2 --use_deepspeed --mixed_precision="bf16" stage2_train_inpaint_model.py \
