...
# 定义文件夹路径
mask_dir = "tryon_stage1/data/VTON/test/agnostic-v3.2"    # 掩码图像所在目录
image_dir = "tryon_stage1/data/VTON/test/image"         # 原始图像所在目录
output_dir = "tryon_stage1/data/VTON/test/warp_mask"            # 输出目录
...
for filename in os.listdir(mask_dir):
    if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp')):
        ...
        cropped_region = cv2.bitwise_and(main_img, contour_mask)
        all_contours = np.vstack(contours)
        x, y, w, h = cv2.boundingRect(all_contours)
        final_cropped = cropped_region[y:y+h, x:x+w]
