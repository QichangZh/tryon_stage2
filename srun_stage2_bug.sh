...
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=4        # 改回4，使用4个MIG实例
#SBATCH --cpus-per-task=30
#SBATCH --mem=800G
#SBATCH --gres=gpu:h100-47:4       # 申请4个MIG实例
...
# 不要手动设置CUDA_VISIBLE_DEVICES，让SLURM处理
# 不要使用unset CUDA_VISIBLE_DEVICES

# 验证识别的GPU情况
echo "======= 验证GPU可见性 ======="
nvidia-smi
python -c "import torch; print(f'PyTorch检测到的GPU数量: {torch.cuda.device_count()}'); [print(f'GPU {i}: {torch.cuda.get_device_name(i)}') for i in range(torch.cuda.device_count())]"

# 使用所有可用的MIG实例启动训练
accelerate launch --multi_gpu --num_processes 4 --use_deepspeed --mixed_precision="bf16" stage2_train_inpaint_model.py \
  --pretrained_model_name_or_path="stabilityai/stable-diffusion-2-1-base" \
  --image_encoder_p_path='facebook/dinov2-giant' \
  --image_encoder_g_path="laion/CLIP-ViT-H-14-laion2B-s32B-b79K" \
  --image_root_path="/home/y/yuansui/tryon_stage1/data/VTON/train"  \
