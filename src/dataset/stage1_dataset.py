...
class PriorImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        s_pose_path = os.path.join(self.image_root_path, item["source_image"]).replace("/train_lst_256_png/", "/normalized_pose_txt/").replace(".jpg", ".txt")
        t_pose_path = os.path.join(self.image_root_path, item["target_image"]).replace("/train_lst_256_png/", "/normalized_pose_txt/").replace(".jpg", ".txt")
